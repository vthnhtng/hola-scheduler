...
function TimeTable() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                // Kiểm tra xem đã có lịch được tạo trước đó chưa
                const checkResponse = await fetch('/api/generate-schedules', {
                    method: 'POST',
                    headers: {
                ...
                // Kiểm tra xem có file được tạo mới không
                const hasNewFiles = data.processedFiles && data.processedFiles.length > 0;
                setHasExistingSchedules(!hasNewFiles);
                ...
                // Lọc lại chỉ lấy teamId cuối cùng
                const onlyTeamIds = timetableResult.teams.map(t => {
                    const parts = t.split('-');
                    return parts[parts.length - 1];
                });
                setTeams([...new Set(onlyTeamIds)]);
                
                console.log('FETCH DATA FROM API', timetableResult.timetableData);
                console.log('Has existing schedules:', !hasNewFiles);
            } catch (error) {
                ...
                <div className="p-6">
                    ...
                    <div className="flex items-center justify-center h-64">
                        <div className="text-center">
                            ...
                            <p className="text-gray-600">Đang tải lịch giảng dạy...</p>
                        </div>
                    </div>
                </div>
            </div>
